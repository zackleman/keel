...
| run-scoped workflow state | `getRunState` and `RunProjection.state` implemented | projection only | projection only | projection snapshot implemented | projection snapshot implemented | `get_state` implemented | `ctx.state` implemented | `run:read`; workflow writes through journaled state effects |
